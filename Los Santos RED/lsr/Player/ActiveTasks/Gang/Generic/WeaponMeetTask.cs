...
{
    public class GangWeaponMeetTask : GangTask, IPlayerTask
    {
        ...
        public bool IsPlayerSellingWeapons { get; set; } = true;

        public GangWeaponMeetTask(ITaskAssignable player, ITimeReportable time, IGangs gangs, IPlacesOfInterest placesOfInterest, ISettingsProvideable settings, IEntityProvideable world,
        ...
        protected override bool GetTaskData()
        {
            ...
            {
                IsAmbush = RandomItems.RandomPercent(Settings.SettingsManager.TaskSettings.WeaponMeetAmbushPercentageNeutral);
            }
            else if (gr.GangRelationship == GangRespect.Friendly)
            {
                IsAmbush = RandomItems.RandomPercent(Settings.SettingsManager.TaskSettings.WeaponMeetAmbushPercentageFriendly);
            }
            if (IsPlayerSellingWeapons)
            {
                UnitPrice = (int)Math.Round(ShopMenus.GetAverageArmsSalesPrice(ModItem) * RandomItems.GetRandomNumber(Settings.SettingsManager.TaskSettings.WeaponMeetPriceScalarMin, Settings.SettingsManager.TaskSettings.WeaponMeetPriceScalarMax));
            }
            else
            {
                UnitPrice = (int)Math.Round(ShopMenus.GetAverageArmsPurchasePrice(ModItem) * RandomItems.GetRandomNumber(Settings.SettingsManager.TaskSettings.WeaponMeetPriceScalarMin, Settings.SettingsManager.TaskSettings.WeaponMeetPriceScalarMax));
            }
            return true;
        }
        ...
        {
            EntryPoint.WriteToConsole("Weapon Meet Loop Start");
            while (true)
            {
        ...
        private void OnSetGangMembersViolent()
        {
            ...
            Game.DisplaySubtitle("Ambush, take them out and get the weapons.");
            EntryPoint.WriteToConsole("WEAPONS MEETUP HAS GONE WRONG, GANG MEMBERS HOSTILE! THEY RECOGNIZE YOU");
        }
        ...
        {
            EntryPoint.WriteToConsole("WEAPON MEETUP PLAYER HAS GONE AWAY FROM LOCATION");
            HasArrivedNearMeetup = false;
            CleanupPeds();
        }
        private void OnArrivedNearMeetup()
        {
            EntryPoint.WriteToConsole($"WEAPON MEETUP PLAYER IS NEAR LOCATION {IsAmbush}");
        ...
        {
            EntryPoint.WriteToConsole("WEAPONS MEETUP START SPAWN DEALERS");
            ...
            if (!IsAmbush)
            {
                EntryPoint.WriteToConsole($"WEAPON MEETUP SPAWNED DEALERS {PrimaryGangMember.Handle} {ModItem.Name} BuyingUnitPrice:{UnitPrice}");
                if (IsPlayerSellingWeapons)
                {
                    PrimaryGangMember.SetupTransactionItems(new ShopMenu("testmenu3", "testmenu4", new List<MenuItem>() { new MenuItem(ModItem.Name, -1, UnitPrice) {
                    ModItem = ModItem,
                ...
                {
                    PrimaryGangMember.SetupTransactionItems(new ShopMenu("testmenu3", "testmenu4", new List<MenuItem>() { new MenuItem(ModItem.Name, UnitPrice, -1) {
                    ModItem = ModItem,
